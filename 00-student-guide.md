...
> This guide explains not just _what_ to do, but _why_ it works. Read it end to end before starting.
...
Running the data generator twice would give you _similar_ but not _identical_ rows, because the script uses randomness. So instead:
...
Open a Query Tool and run **`01-schema-hospital-slow.sql`**.
...
In pgAdmin, disconnect from `hospital_slow` first. Right-click it in the tree and pick _Disconnect Database_. Any Query Tool tab pointing at it also counts as a connection, so close those.
...
If you get `source database "hospital_slow" is being accessed by other users`, you still have a session open to it. Right-click it, _Disconnect Database_, close any open Query Tool tab, then try again.
